...
async function loadResult() {
  ...
  if (err) {
    ...
    if (filter.period === 'daily') {
      resultTitle.value = `Laporan Stok Opname ${formatDate(filter.date, 'DD MMMM YYYY')}`;
    } else {
      resultTitle.value = `Laporan Stok Opname Bulan ${months[filter.month]} ${filter.year}`;
    }
...
async function onExport(type = 'csv') {
  ...
  const [, err] = await request(`/api/v1/stock-opnames/-actions/export`, {
    method: 'post',
    body: {
...
function onSuccesExport(e) {
  if (e.type === 'App\\Notifications\\ReportStockOpnameExported') {
    exportLoading.value = false;
...
<template>
  <BaseHeading>Laporan Stok Opname</BaseHeading>

  <BaseCard title="Form Laporan Stok Opname">
