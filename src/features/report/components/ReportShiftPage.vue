...
const title = computed(() => {
  ...
  if (filter.period === 'daily') {
    return `Laporan Shift ${formatDate(filter.date, 'DD MMMM YYYY')}`;
  }

  if (filter.period === 'monthly') {
    return `Laporan Shift Bulan ${months[filter.month]} ${filter.year}`;
  }

  return `Laporan Shift Tahun ${filter.year}`;
});
